...
def html_table(df: pd.DataFrame, tid: str) -> str:
    ...
    for _, row in df.iterrows():
        ...
        cells = "".join(
            f'<td class="{"num" if c in num_cols else ""}">{html.escape(_fmt(v))}</td>'
...
def build_page(ctx: dict) -> str:
    ...
    return f"""<title>Ayenati Test-Wise Statistics</title>
...
<div class="page">
  <header>
    ...
    <p class="lede">{html.escape(ctx['hospital'])} &nbsp;&middot;&nbsp; {html.escape(ctx['period'])}
      &nbsp;&middot;&nbsp; source <code>{html.escape(ctx['source_name'])}</code></p>
    <p class="filters">Filters: <code>Is external lab order = Yes</code> &nbsp;<b>and</b>&nbsp;
      ...
      grouped by cleaned <code>Test description</code>.</p>
  </header>
